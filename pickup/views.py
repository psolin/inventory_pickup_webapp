...
def trash_item(request):
    if request.GET.get('status') == "trash":
        ...
        item_count = len(
            Item.objects.filter(transaction_num=pulled_transaction))
...
def add_note(request):
    if request.GET.get('status') == "add_note":
        ...
        new_note = Note(transaction_num=pulled_transaction, content=blurb,
                        date=now, user=pulled_user)
        new_note.save()
        ...
        note_count = len(
            Note.objects.filter(transaction_num=pulled_transaction))

        return latest_note_date.strftime(
            '%A, %b. %d, %Y, %I:%m %p'), note_count, note_user
...
def edit_transaction(request):
    if request.GET.get('status') == "edit_transaction":
        ...
        if transaction_number != edited_transaction_number:
            ...
            cursor = connection.cursor()
            update_notes_query = "UPDATE 'note' SET transaction_num = %s WHERE transaction_num = %s" % (
            edited_transaction_number, transaction_number)
...
def check_item(request):
    if request.GET.get('status') == "check_item":
        ...
        if pulled_item.picked_up_on == None:
            ...
            cursor = connection.cursor()
            cursor.execute(
                "UPDATE 'item' SET picked_up_on = NULL WHERE itemid = %s",
                [int(item_to_check)])
        ...
        return item_status, current_active, transaction_info(
            transaction_number), item_date
